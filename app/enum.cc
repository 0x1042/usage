...
TEST(enum, tostr) {
    Color c1 = Color::RED;
    INFO("color name is {}", magic_enum::enum_name(c1));
    magic_enum::enum_for_each<Color>([](auto val) -> void {
        constexpr Color color = val;
        INFO("color name is {}", magic_enum::enum_name(color));
    });
}
...
TEST(enum, fromstr) {
    ...
    auto color = magic_enum::enum_cast<Color>(color_name, magic_enum::case_insensitive)
                     .value_or(Color::NONE);
    INFO("success parsed. {}", magic_enum::enum_name(color));
}
...
TEST(enum, tointeger) {
    ...
    magic_enum::enum_for_each<Color>([](auto val) -> void {
        constexpr Color color = val;
        INFO("color_integer is {}", magic_enum::enum_integer(color));
    });
}
